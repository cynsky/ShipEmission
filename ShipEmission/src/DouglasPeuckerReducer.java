...
/** 
 * the number of points in a shape using the Douglas-Peucker algorithm. 
 * From:
...
public class DouglasPeuckerReducer {
	 ...
	 */
	public static ArrayList<GeoPoint> reduceWithTolerance(
			ArrayList<GeoPoint> shape, double tolerance) {
		int n = shape.size();
		...
		ArrayList<GeoPoint> newShape = new ArrayList<GeoPoint>(n); // the new
																	// shape to
																	// return
		for (int i = 0; i < n; i++) {
			if (marked[i])
	 ...
	 *            this line segment
	 */
	private static void douglasPeuckerReduction(ArrayList<GeoPoint> shape,
	 ...
	 */
	public static double orthogonalDistance(GeoPoint point, GeoPoint lineStart,
			GeoPoint lineEnd) {
		double area = Math.abs((1.0 * lineStart.latitudeE6
				* lineEnd.longitudeE6 + 1.0 * lineEnd.latitudeE6
				* point.longitudeE6 + 1.0 * point.latitudeE6
				* lineStart.longitudeE6 - 1.0 * lineEnd.latitudeE6
				* lineStart.longitudeE6 - 1.0 * point.latitudeE6
				* lineEnd.longitudeE6 - 1.0 * lineStart.latitudeE6
				* point.longitudeE6) / 2.0);

		double bottom = Math.hypot(lineStart.latitudeE6 - lineEnd.latitudeE6,
